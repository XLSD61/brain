...
public class BrainManager : MonoBehaviour
{
    ...
    public void AddBrainChapters(GameRoot gameRoot, float value)
    {
        if (gameRoot == GameRoot.FollowTheLeader)
        {
            AddMemory(value);
            AddSpeed(value);
        }
        else if (gameRoot == GameRoot.TapTheColor)
        {
            AddMemory(value);
            AddObservatýon(value);
        }
        else if (gameRoot == GameRoot.SpinningBlcok)
        {
            AddObservatýon(value);
            AddDecision(value);
        }
        else if (gameRoot == GameRoot.SimpliCity)
        {
            AddCalculation(value);
            AddSpeed(value);
        }
        else if (gameRoot == GameRoot.Concentration)
        {
            AddMemory(value);
            AddObservatýon(value);
        }
